...
def saveFile():
    new_file = asksaveasfile(mode = 'w', filetype = [('text files', '.txt'), ('ASCII', '.java')])
    if new_file is None:
        return
...
def openFile():
    file = askopenfile(mode = 'r', filetype=[('text files', '.txt'), ('ASCII', '.java')])
    if file is not None:
        content = file.read()
...
entry = tk.Text(canvas, wrap="word", bg="#CDC8B1", font=("poppins", 15))
